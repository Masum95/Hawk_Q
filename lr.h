...
// return Feature X 1 matrix 
MatrixXd linear_regression(const MatrixXd &X, const MatrixXd &Y)
{

	MatrixXd weight(X.cols(),1);	 // feature X 1 matrix 
	for (int i = 0; i < (int)weight.size(); ++i)
	{
		weight(i, 0) = 0.0;
	}

	MatrixXd X_T = X.transpose(); // f X N matrix 

	MatrixXd A = X_T * X; // f X N * N X f = f X f
	if (A.determinant() == 0)
		return weight;
	MatrixXd A_inv = A.inverse();

	MatrixXd B = A_inv * X_T; //  f X f * f X N = f X N 

	weight = B * Y; // f X N * N * 1 = f X 1 
...
//Log-likelihood of multivaritate linear regression
// Formula = = − 1/2 Σ ( y_i - B'x_i)' Σ^-1  ( y_i - B'x_i) + c
...
MatrixXd covariance_matrix(const MatrixXd &X)
{ // create a copy of dataset. it will be modified
	MatrixXd centered = X.rowwise() - X.colwise().mean();
	MatrixXd cov = (centered.transpose() * centered) / double(X.rows());
	return cov;
}
