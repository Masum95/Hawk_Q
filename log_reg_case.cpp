...
# run with -t thread_cnt -p pc_cnt -c 
int main(int argc,char **argv)
{
    ...
    unsigned int nrow = find_row_count(); #number of samples 
...
void * worker_thread_func(void *arg)
{
	...
	while(true)
	{
		...
		for(int l=thread_no;l<read_row_count;l+=interleave) // loop over kmer
		{
			...
			for(int dat = 0; dat < thread_local_features_ALT.size(); ++dat){ // loop over sample 
				std::vector<double> data(thread_local_features_ALT[0].size());
				for(int j = 0; j<thread_local_features_ALT[0].size(); ++j){ // size = 4/5 
					data[j] = thread_local_features_ALT[dat][j];
				}
				double p = predict(alt_model, data); # given this dataset how good the alt model fit  
